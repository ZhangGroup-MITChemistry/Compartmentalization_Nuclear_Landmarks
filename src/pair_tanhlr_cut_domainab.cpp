...
void PairTanhlrCutDomainab::compute(int eflag, int vflag)
{
  ...
  double r2inv,r6inv,sig12lad,sig6lad,sig12nad,sig6nad,sig12spec,sig6spec;  //Used in LJ potential interaction if turned on
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      /// *** The above else if was generalised to suit the changing number of nucleolus because 547 is hard code molecule id of first lamina particle with 500 nucleolus particles*** ///  
        ...
        //LAD portion
        if(alpha!=0.0){
        utanh += subcomp[chrbead][0]*4.0*alpha*r6inv*(sig12lad*r6inv-sig6lad);
        fpair += subcomp[chrbead][0]*factor_lj*4.0*alpha*r6inv*((12.0*sig12lad*r6inv)-(6.0*sig6lad))*r2inv;}  
        //NAD portion 
        if(beta!=0.0){
...
{ //This function seems to be never executed in the run
  ...
  r=sqrt(rsq);
  //why isn't the global cutoff important here
  //Kartik edit
  if (imol<47 && jmol <47){
  if (r <= rmh[itype][jtype]) {
      rexp = (rmh[itype][jtype]-r)*sigmah[itype][jtype];
  ...
  //lamina particle interact with chr via only tanh
  else if (((imol>46) && (jmol<46)) || ((imol<46) && (jmol>46)) ){
    rexp = (rmh[itype][jtype]-r)*sigmah[itype][jtype];
    tanr = tanh(rexp);
    utanh = ptanh[iscale][itype][jtype]*(1.0 + tanr);
    fforce = factor_lj/r*ptanh[iscale][itype][jtype]*(1.0-tanr*tanr)*sigmah[itype][jtype];
  ...
  //the offset is correctly determined from the atom type
  //printing just to check if this function is executed
  printf("Code block within Single");
